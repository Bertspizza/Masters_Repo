...
def plot_ami_multiple(ami_data, condition, column, save_image=False, file_path=None, title=None):
    plt.figure(figsize=(10, 6))
    
    for participant_id, ami_result_list in ami_data.items():
        # ami_result_list is a list of AMI results per file processed for this participant and column
        for ami_result in ami_result_list:
            ami_result = np.array(ami_result)
            plt.plot(ami_result[:, 0], ami_result[:, 1], label=f'{participant_id}')
    ...
    if title is not None:
        ...
        plt.title(f'Average Mutual Information - Condition: {condition}, Column: {column}')
    plt.legend(loc='best', fontsize='small')
    plt.xticks(np.arange(0, max(ami_result[:, 0]) + 10, 10))
